...
ch_http_response_t *ch_http_simple_query(ch_http_connection_t *conn, const char *query)
{
	...
	curl_easy_setopt(conn->curl, CURLOPT_TCP_KEEPALIVE, 1);
	curl_easy_setopt(conn->curl, CURLOPT_TCP_KEEPIDLE, 50);
	curl_easy_setopt(conn->curl, CURLOPT_TCP_KEEPINTVL, 20);
